...
`theme` is the serving pack's `{ id, version }`; `entry` and `css` come from
that pack. Optional fields: `shared`, `layout`, `redirect`, `head`. `path` is
absolute. `page` and `entry` are non-blank.
