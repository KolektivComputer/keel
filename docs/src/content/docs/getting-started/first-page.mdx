...
<a href={path("/docs/getting-started/host")}>the host</a> and
<a href={path("/docs/getting-started/pack")}>the pack</a>.

This is the one-pack shape: you own the host and the frontend, and pass that
one pack at render. The same registry is the contract other packs would
implement if you later install them. See <a href={path("/docs/core-concepts/contracts")}>Page contracts</a>.
...
The sample below is Svelte (`page()`, `<Head />`). Change the navbar frontend picker for the matching adapter API (`usePage`, `injectKeelPage`, and so on). See <a href={path("/docs/implementing/framework-adapters")}>Framework adapters</a>.
</FrameworkPanel>
...
Use the TS/JS toggle on the block itself to pin a snippet, or the language
control in the navbar to switch every example that has not been pinned.
