import type { PageHead } from "./types.ts"

/** Marker attribute on every head node Keel creates or adopts. */
export const KEEL_HEAD_ATTR = "data-keel-head"
...
export function applyHead(head: HeadInput): () => void {
  ...
  if (head.title) {
    ...
    if (title) {
      title.setAttribute(KEEL_HEAD_ATTR, "")
...
function clearKeelHead(): void {
  if (typeof document === "undefined") return
  for (const el of [...document.head.querySelectorAll(`[${KEEL_HEAD_ATTR}]`)]) {
...
function upsert(tag: string, attrs: Record<string, string>, created: Element[]): void {
  ...
  el.setAttribute(KEEL_HEAD_ATTR, "")
