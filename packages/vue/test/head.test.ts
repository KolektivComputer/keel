...
function renderHead(props: Record<string, unknown> = {}, slot?: () => unknown) {
  return render(
    defineComponent({
      setup: () => () => (slot ? h(Head, props, { default: slot }) : h(Head, props)),
    }),
  )
}
...
test("Head applies explicit props and cleans up on unmount", async () => {
  setPage(seed("/"))
  const view = await renderHead({ title: "Explicit", description: "Explicit description" })
...
test("Head renders children into document.head through a teleport", async () => {
  setPage(seed("/"))
  const view = await renderHead({}, () => h("meta", { name: "custom", content: "x" }))
