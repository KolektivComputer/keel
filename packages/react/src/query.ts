 ...
 *
 * Returns the TanStack mutation directly — read `isPending` and `error` in
 * render, call `mutateAsync` from event handlers.
 */
export function useAction<I = unknown, O = unknown>(
  id: string,
  options: UseActionOptions = {},
): UseMutationResult<O, ActionError, I> {
 ...
 * seed.
 */
export function useKeelPageQuery<T = unknown>(): UseQueryResult<KeelSeed<T>, Error> {
  const queryClient = getQueryClient()
