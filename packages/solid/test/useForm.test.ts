...
function mountForm(): { probe: Probe; unmount(): void } {
  ...
  return { probe, unmount: view.unmount }
}

test("useForm tracks dirtiness, set, reset, and clearErrors", () => {
  const { probe, unmount } = mountForm()
